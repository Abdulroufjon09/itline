<template>
  <div class="min-h-screen bg-slate-50 p-4 sm:p-6 font-sans">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
        ...
        >
          ...
          <div class="relative">
            ...
            <p
              class="text-xl sm:text-2xl font-bold text-slate-800 tabular-nums break-words"
            >
              {{ fmt(summary.collected_total) }}
            </p>
            <p
              class="text-xs text-emerald-500 font-medium mt-1.5 flex items-center gap-1"
            >
              <span
        ...
        >
          ...
          <div class="relative">
            ...
            <p
              class="text-xs text-rose-400 font-medium mt-1.5 flex items-center gap-1"
            >
              <span
                class="inline-block w-1.5 h-1.5 rounded-full bg-rose-400"
              ></span>
        ...
        <!-- Sof foyda/zarar -->
        <div
          class="rounded-2xl p-5 relative overflow-hidden shadow-sm"
          :class="
            summary.profit >= 0
        ...
        >
          <div
            class="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-bl-full"
          ></div>
          <div class="relative">
            <div class="flex items-center gap-2 mb-4">
              <div
              ...
              <span
                class="text-xs font-medium text-white/70 uppercase tracking-wider"
              >
                {{ summary.profit >= 0 ? "Sof foyda" : "Sof zarar" }}
              </span>
            </div>
            <p
              class="text-xl sm:text-2xl font-bold text-white tabular-nums break-words"
            >
      ...
      <div
        class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 mb-5"
      >
        <h2
        ...
        <div class="grid grid-cols-2 lg:grid-cols-5 gap-3">
          ...
          <div class="bg-emerald-50 rounded-xl p-3.5 text-center">
            <p
              class="text-lg sm:text-xl font-bold text-emerald-700 tabular-nums"
            >
              {{ summary.paid_count }}
            </p>
          ...
          <div
            class="bg-indigo-50 rounded-xl p-3.5 text-center col-span-2 lg:col-span-1"
          >
            <p
              class="text-lg sm:text-xl font-bold text-indigo-600 tabular-nums"
            >
              {{
                summary.total_students
                  ? Math.round(
      ...
      <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
        <div
          class="flex gap-1 bg-white border border-slate-100 shadow-sm rounded-xl p-1"
        >
          <button
            v-for="tab in tabs"
            :key="tab.key"
      ...
      <div
        v-if="activeTab === 'expenses' && showExpenseForm"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          <div>
            ...
            <input
              ...
              type="text"
              placeholder="Masalan: Internet"
              :class="
                expenseErrorFields.has('title')
        ...
        <table class="w-full min-w-[640px]">
          <thead>
            <tr class="border-b border-slate-100 bg-slate-50/70">
              <th
                class="text-left text-xs font-semibold text-slate-400 uppercase tracking-wider px-5 py-3.5"
              >
                Talaba
              </th>
              <th
          ...
          <tbody class="divide-y divide-slate-50">
            ...
            >
              ...
              <td
                class="px-5 py-3.5 text-sm font-bold tabular-nums text-slate-700"
              >
                {{ fmt(p.amount_due) }}
              </td>
              <td
                class="px-5 py-3.5 text-sm font-bold tabular-nums"
                :class="
                  paidAmountOf(p) > 0 ? 'text-emerald-600' : 'text-slate-300'
                "
              >
                {{ fmt(paidAmountOf(p)) }}
              </td>
              <td
                class="px-5 py-3.5 text-sm font-bold tabular-nums"
                :class="
                  remainingOf(p) > 0 ? 'text-red-500' : 'text-emerald-600'
                "
              >
                {{ remainingOf(p) > 0 ? "-" : ""
              ...
              <td class="px-5 py-3.5">
                ...
                >
                  <span
                    ...
                    :class="p.is_paid ? 'bg-emerald-400' : 'bg-amber-400'"
                  ></span>
        ...
        <table class="w-full min-w-[560px]">
          ...
          <tbody class="divide-y divide-slate-50">
            ...
            >
              ...
              <td
                class="px-5 py-3.5 text-sm font-bold text-rose-500 tabular-nums"
              >
                −{{ fmt(e.amount) }}
              </td>
              <td class="px-5 py-3.5">
