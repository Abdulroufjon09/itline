...
/** Generic fetch wrapper — HTML error page bo'lsa crash qilmaydi */
...
async function fetchMonthlyAbsences(studentIds) {
  // Faqat cache da yo'q student ID larni yuklaymiz
      ...
      uncached.map(async (id) => {
        try {
          ...
          const count = Array.isArray(data)
            ...
            : 0;
          absenceCache.value[`${id}_${currentMonth}`] = count;
        } catch {
  ...
  // absenceMap ni cache dan to'ldirish
  const map = {};
...
async function setStatus(att, status) {
  ...
  try {
    ...
    // Absence counter ni lokal yangilash (cache ham yangilanadi)
    const sid = att.student_id;
    const cacheKey = `${sid}_${currentMonth}`;
    if (prev === "absent" && status !== "absent") {
      const newVal = Math.max(0, (absenceMap.value[sid] || 0) - 1);
      absenceMap.value[sid] = newVal;
      absenceCache.value[cacheKey] = newVal;
    } else if (prev !== "absent" && status === "absent") {
      const newVal = (absenceMap.value[sid] || 0) + 1;
      absenceMap.value[sid] = newVal;
      absenceCache.value[cacheKey] = newVal;
    }
  } catch (e) {
